...
> Измерение программного обеспечения, Software Measurement

软件开发过程 - 软件测量

本节探讨了软件过程和产品测量、测量结果的质量、软件信息模型和软件过程测量技术。

在引入一个新的过程或改变一个当前的过程之前，应该获得当前情况的测量结果，以便为当前情况和新情况的比较提供依据。
例如，在引入一个软件检查过程之前，应该测量纠正测试期间发现的缺陷所需的努力。

在最初的启动期和软件检查过程实施后，可以将检查和测试的综合努力与以前只需要测试的努力进行比较。
如果流程被修改，类似的考虑也适用。

**软件过程和产品测量 (Software Process and Product Measurement)**

- 软件和产品开发的测量涉及到确定一个过程、活动或任务的有效性和效率。

- 软件过程、活动或任务的**效率**是实际消耗的资源与执行软件过程、活动或任务时预期或希望消耗的资源的比率。

    **效率**是指一个过程、行动或任务的实际结果与预期结果的比率。

    例如，在软件测试过程中发现和纠正的实际缺陷数与预期发现和纠正的缺陷数（例如，从类似项目的历史数据中计算）。

    请注意，测量过程绩效需要测量相关的产品属性；例如，测量在软件测试期间发现和纠正的软件缺陷。

    在测量产品属性以确定过程性能时，应谨慎行事。

    例如，通过测试发现和纠正的缺陷数量可能达不到预期的缺陷数量，从而提供了一个具有欺骗性的低效衡量标准

    这可能是因为被测软件的质量好于正常质量，或者是因为，例如，最近引入的软件检查过程减少了软件中的残余缺陷数量。

    在确定软件过程的有效性方面可能很重要的产品指标包括产品的复杂性、缺陷总数、缺陷密度以及需求、设计文件和其他相关工作产品的质量。

    **效率和效果是独立的概念。**

    - 一个高效软件开发过程在实现过程的预期结果方面可能是低效的；例如，花在发现和修复软件缺陷上的工作量可能非常大，导致与预期相比效率很低。
    - 一个高效的过程可能在实现输入工作产品向输出工作产品的预期转换方面效率低下；例如，在测试过程中无法发现和修复足够的软件缺陷。
    - 软件开发过程、活动或任务的执行方式效率低下和/或表现不佳的原因可能包括以下一个或多个方面：输入工作产品不足、工作人员经验不足、缺乏足够的工具和基础设施、学习新的过程、复杂的产品或不熟悉的产品领域。

- 对于大多数软件开发过程、活动和任务来说，努力（或同等成本）是衡量资源的基本标准；

- 以工时、工日、工周或工月等单位衡量，或以欧元、美元、卢布等等价货币单位衡量。
