...
- ==类图和对象图 (диаграммы классов и объектов | сlass and object diagrams)==
...
- ==部署图 (Диаграммы развертывания | Deployment diagrams)==
...
- ==结构图 (Структурные диаграммы | structure charts)==
...
- 活动图 (Диаграммы действий | Activity diagrams)

    用于显示从行动到行动的控制流。它们可以用来表示同时进行的行动

    

- 通信图 (Диаграммы связей | communication diagrams)
...
- 决策表和图表 (Таблицы решений и диаграммы - decision tables and diagrams))

    用于表示条件和行动的复杂组合

    

- 流程图 (Блок-схемы  - Flowcharts)

    用于表示控制的流程和要执行的相关行动

    

- 顺序图 (Диаграммы последовательности - sequence diagrams)

    用来表示一组对象之间的互动，重点是对象之间传递信息的时间顺序



- 状态转换图和状态图 (Диаграммы перехода между состояниями и диаграммы состояний | state transition and state chart diagrams)

    用于表示从一个状态到另一个状态的控制流，以及一个组件的行为如何根据其在最终自动机中的当前状态而变化

    

- 形式化规范语言 (Формальные языки спецификации)
